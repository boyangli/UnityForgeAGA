...
public class WorldGUI : MonoBehaviour {
    ...
    public void OnGUI() {
        ...
        this.dialogueBounds = new Rect(
            ...
            this.DialogueWidth,
            Screen.height / 2 - 2 * this.DialogueOffset.y);
        ...
        if (this.DisplayEpisodes) GUI.Window(0, this.episodeBounds, this.DrawEpisodes, "", this.defaultSkin.window);
        if (this.displayDialogueBuffer) GUI.Window(1, this.dialogueBounds, this.DrawDialogue, "");
        if (this.displayInventoryBuffer) GUI.Window(2, this.inventoryBounds, this.DrawInventory, "");
        if (this.displayStoryBuffer) GUI.Window(3, this.storyBounds, this.DrawStory, "");
    ...
    private void DrawEpisodes(int windowID) {
        ...
        if (load && this.selectedEpisode != -1) {
            ...
            if (this.validEpisode) this.DisplayEpisodes = false;
        }
    }
    ...
    private string[] GetEpisodeNames() {
        ...
        if (this.externalEpisode) {
            ...
            if (!exists) Directory.CreateDirectory("Episodes");
            ...
            for (int i = 0; i < episodes.Length; i++) names[i] = episodes[i].name;
        }
        return names;
    }
    ...
    private void DrawDialogue(int windowID) {
        ...
        //GUILayout.Space(8);
        GUILayout.BeginVertical();
        GUILayout.Label(this.Dialogue.Speaker.Name);
        GUILayout.Label("", "Divider");
        GUILayout.Label(this.Dialogue.Line, "LightText");
    ...
    private void DrawStory(int windowID) {
        ...
        //GUILayout.Space(8);
        GUILayout.BeginVertical();
        GUILayout.Label("Episode Script");
        ...
        foreach (Task task in dm.Script) {
            string s = task.Actor.Name + ":  " + task.Description;
            //Indicate the current task with blue text.
            if (task == dm.CurrentTask) {
